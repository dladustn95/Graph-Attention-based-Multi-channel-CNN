...
class ConvKB(nn.Module):
    def __init__(self, input_dim, input_seq_len, in_channels, out_channels, drop_prob, alpha_leaky):
        ...
        self.conv_layer = nn.Conv2d(
            in_channels, out_channels, (3, input_seq_len))  # kernel size -> 3*input_seq_length(i.e. 2)
        self.dropout = nn.Dropout(drop_prob)
        self.non_linearity = nn.ReLU()
        self.fc_layer = nn.Linear((input_dim-2) * out_channels, 1)
    ...
    def forward(self, conv_input):
        ...
        batch_size, input_channel, length, dim = conv_input.size()
