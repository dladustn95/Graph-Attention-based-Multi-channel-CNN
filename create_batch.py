...
class Corpus:
    ...
    def get_validation_pred(self, args, model, unique_entities):
        now = time.localtime()
        f = open((args.output_folder + "evaluate_conv_{}-{}-{}.txt").format(now.tm_year,now.tm_mon, now.tm_mday), 'w')
        ...
        for iters in range(1):
            ...
            f.write("\ntest set length ")
            f.write(str(len(self.test_indices)))
            ...
            for i in range(batch_indices.shape[0]):
                ...
                f.write("\nsample - ")
                f.write(str(ranks_head[-1]))
                f.write(str(ranks_tail[-1]))
        ...
        f.write("\n\nCumulative stats are -> ")
        f.write("\nHits@100 are {}".format(cumulative_hits_100))
        f.write("\nHits@10 are {}".format(cumulative_hits_ten))
        f.write("\nHits@3 are {}".format(cumulative_hits_three))
        f.write("\nHits@1 are {}".format(cumulative_hits_one))
        f.write("\nMean rank {}".format(cumulative_mean_rank))
        f.write("\nMean Reciprocal Rank {}".format(cumulative_mean_recip_rank))
        f.close()
