...
def train_gat(args):
    ...
    for epoch in range(args.epochs_gat):
        ...
        summary.add_scalar('loss/gat_loss', sum(epoch_loss) / len(epoch_loss), epoch)
    ...
    now = time.localtime()
    f = open((args.output_folder + "train_gat_epoch_losses_{}-{}-{}.txt").format(now.tm_year,now.tm_mon, now.tm_mday), 'w')
...
def train_conv(args):
    ...
    for epoch in range(args.epochs_conv):
        ...
        for iters in range(num_iters_per_epoch):
            ...
            summary.add_scalar('loss/conv_loss_iter', loss.data.item(), iters + epoch*num_iters_per_epoch)
        ...
        if epoch%10==0:
            ...
            epoch, sum(epoch_loss) / len(epoch_loss), time.time() - start_time))
        epoch_losses.append(sum(epoch_loss) / len(epoch_loss))
        summary.add_scalar('loss/conv_loss_epoch', sum(epoch_loss) / len(epoch_loss), epoch)
    ...
    now = time.localtime()
    f = open((args.output_folder + "train_conv_epoch_losses_{}-{}-{}.txt").format(now.tm_year,now.tm_mon, now.tm_mday), 'w')
    for i in epoch_losses:
        f.write(str(i))
        f.write('\n')
    f.close()
